...
interface OrdersDao {
    ...
    fun selectOrderY(year: Int) : ArrayList<ModelSale>

    @Query("SELECT * FROM `order` WHERE month=:month AND year=:year ORDER BY day ASC")
    fun selectOrdersM(month: Int,year: Int) : ArrayList<ModelSale>

    @Query("SELECT * FROM `order` WHERE day = :day AND month=:month AND year=:year ")
    fun selectOrderD(day: Int, month: Int, year: Int) : ArrayList<ModelSale>

    @Query("SELECT * FROM `Order` ORDER BY year ASC")
    fun selectOrderAll() : ArrayList<ModelSale>
